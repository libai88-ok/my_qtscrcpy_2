...
QYUVOpenGLWidget::~QYUVOpenGLWidget()
{
    ...
    //所以makeCurrent，doneCurrent开启，关闭openGl上下文
    //在initializeGL、resizeGL、paintGL三函数中默认在openGL上下文中
    makeCurrent();
    m_vbo.destroy();
    deInitTextures();
    doneCurrent();
...
//在YUV中，通常是yyyyuuvv存储；但是为了ffmpeg为了对齐会填充yyyyeeuuevve；
//linesizeY即为包含填充e的长度
...
{
    //Qt为opengl实现了三种：AA_UseDesktopOpenGL使用真正的opengl与显卡驱动，
    //Qt中实现的opengl接口AA_UseOpenGLES，AA_UseSoftwareOpenGL使用软件模拟
...
void QYUVOpenGLWidget::updateTexture(GLuint texture, quint32 textureType, quint8 *pixels, quint32 stride)
{
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, size.width(), size.height(), GL_RED, GL_UNSIGNED_INT, pixels);
    doneCurrent();
